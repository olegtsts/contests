        ...
        if (event.type == RECIPE_OPEN) {
            ...
            if (current_recipies == k) {
                for (auto& index: active_query_indices) {
                    counts[index] += event.coord - std::max(last_seen, queries[index].first - 1);
                }
            }
            --current_recipies;
        } else if (event.type == QUERY_CLOSE) {
            if (current_recipies >= k) {
                counts[event.index] += event.coord - std::max(last_seen, queries[event.index].first - 1);
            }
