...
public:
    ...
    std::tuple<int, int, int> GetBonusAndPassedTime(const size_t car_index, const size_t track_index) {
        ...
        return std::make_tuple(total_bonus, total_passed_time, track_length);
    }
    long double GetFunctionalValue(const size_t car_index, const size_t track_index) {
        int total_bonus, total_passed_time, track_length;
        std::tie(total_bonus, total_passed_time, track_length) = GetBonusAndPassedTime(car_index, track_index);
        return (long double)(total_bonus > 0) * static_cast<long double>(track_length) / static_cast<long double>(total_passed_time);
    }
    void MoveCar(const size_t car_index, const size_t track_index) {
        int total_passed_time;
        std::tie(std::ignore, total_passed_time, std::ignore) = GetBonusAndPassedTime(car_index, track_index);
        Car& car = cars[car_index];
        Track& track = tracks[track_index];
    ...
    void Process() {
        ...
        std::set<std::pair<int, size_t>> lengths_and_indices;
        for (size_t track_index = 0; track_index < tracks.size(); ++track_index) {
            auto& track = tracks[track_index];
            if (
                (track.start_position.x >= 4500 || track.finish_position.x >= 4500 || track.start_position.y >= 3300 || track.finish_position.y >= 3300)
            ) {
                lengths_and_indices.insert({GetDistance(track.start_position, track.finish_position), track_index});
            }
        }
        size_t how_many_to_remain = 175;
        for (size_t i = 0; i < how_many_to_remain; ++i) {
            lengths_and_indices.erase(*lengths_and_indices.rbegin());
        }
        for (auto& this_pair: lengths_and_indices) {
            skip_tracks[this_pair.second] = true;
        }
